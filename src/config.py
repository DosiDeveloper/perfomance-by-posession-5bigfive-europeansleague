...
class Config:
    def __init__(self):
        ...
        self.folder_data_path = os.getenv('FOLDER_DATA_PATH', str(Path(__file__).parent.parent / "Data"))
        self.folder_data_state_path = {    
            "Clean": Path(self.folder_data_path) / "Clean",
            "Processed": Path(self.folder_data_path) / "Processed",
        }
        self.db_path = os.getenv('DB_PATH', f'{self.folder_data_path}/db.sqlite3')
